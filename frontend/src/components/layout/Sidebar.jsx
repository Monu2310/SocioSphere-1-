...
export default function Sidebar({ mobileOpen = false, onClose = () => {} }) {
  ...
  return (
    <aside
      className={`sidebar-shell motion-surface fixed inset-y-0 left-0 z-40 flex h-screen flex-col border-r border-white/10 bg-dark-800/80 backdrop-blur-xl transition-transform duration-300 lg:static lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} ${collapsed ? 'sidebar-collapsed w-16' : 'w-72 lg:w-64'}`}
    >
      {/* Logo */}
      <div className={`sidebar-header flex items-center gap-3 p-4 ${collapsed ? 'justify-center' : ''}`}>
      ...
      {/* User Role Badge */}
      {!collapsed && (
        <div className="sidebar-profile mx-3 mt-3 p-3 glass-card">
      ...
      {/* Navigation */}
      <nav className="sidebar-nav flex-1 p-3 space-y-1 mt-2 overflow-y-auto">
      ...
      <div className="p-3 border-t border-white/10 space-y-1">
        <NavLink to={isAdmin ? '/admin/notifications' : '/notifications'} className={({ isActive }) => isActive ? 'sidebar-link sidebar-link-active' : 'sidebar-link'} onClick={onClose}>
          <Bell size={18} className="flex-shrink-0" />
          {!collapsed && <span className="text-sm">Notifications</span>}
        </NavLink>
        <NavLink to="/profile" className={({ isActive }) => isActive ? 'sidebar-link sidebar-link-active' : 'sidebar-link'} onClick={onClose}>
          <Settings size={18} className="flex-shrink-0" />
          {!collapsed && <span className="text-sm">Profile</span>}
        </NavLink>
        <button onClick={handleLogout} className="sidebar-link sidebar-action w-full text-red-400 hover:text-red-300 hover:bg-red-500/10">
      ...
      <button
        ...
        className="sidebar-collapse-toggle absolute right-2 top-16 hidden items-center justify-center transition-colors z-20 lg:flex"
      >
